...
class Player(Tk.Frame):
    ...
    def Download_Trailer(self,name_film):
        try:
            ...
            pass
        #search_url = "https://www.youtube.com/results?search_query=cinema trailer "+name_film+"&sp=EgIYAQ%253D%253D"
        search_url = "https://www.youtube.com/results?search_query=english+cinema+trailer+"+name_film
...
def reproduce_trailer_film(name_film):
...
if __name__ == "__main__":
    ...
    reproduce_trailer_film('_Indiana_Jones_and_the_Last_Crusade_(1989)')
'''
...
'''
    ...
    root = Tk.Tk()
    player = Player(root, video=_video)
    player.Download_Trailer('_Indiana_Jones_and_the_Last_Crusade_(1989)')
    video_path = ('./film/'+ (os.listdir('./film/')[0]))
    player._Play(video_path)
    root.protocol("WM_DELETE_WINDOW", player.OnClose)  # XXX unnecessary (on macOS)
    root.mainloop()
